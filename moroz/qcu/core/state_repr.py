...
class IQPUConfig:
    ...
    Tphi: np.ndarray | None = None

    phi_ref: float = 0.30
    eps_drive: list[complex] | None = None

    t_max: float = 10.0
    dt: float = 0.05
    obs_every: int = 1

    qubits_init: list[str] | None = None
    alpha0: list[complex] | None = None
...
class IQPURunResult:
    ...
    dtheta_end: float
    N_end: float | None
    final_rho: np.ndarray | None = None  # 末态密度矩阵，供下一 gate 作初态

...
def tune_with_utm(
    ...
    # 只调评分/权重参数，硬约束 (max_single_weight, cash_floor) 不进搜索空间
    param_names = ["w_mom", "w_trend", "w_vol", "w_volume", "w_dd", "lambda_risk", "persistence_bonus", "max_assets"]
    means = np.array([0.9, 0.2, 0.33, 0.12, 0.36, 0.42, 0.13, 3.0], dtype=float)
    scales = np.array([0.45, 0.18, 0.18, 0.08, 0.18, 0.20, 0.08, 0.9], dtype=float)
    lower = np.array([0.1, -0.2, 0.01, -0.2, 0.01, 0.01, 0.0, 1.0], dtype=float)
    upper = np.array([2.5, 1.2, 1.0, 0.5, 1.5, 1.5, 0.4, 5.0], dtype=float)
    # 硬约束固定值，UTM 不碰
    fixed_constraints = {"cash_floor": 0.10, "max_single_weight": 0.50}
    ...
    for gen in range(generations):
        ...
        for _ in range(population):
            ...
            d.update(fixed_constraints)  # 硬约束不被覆盖

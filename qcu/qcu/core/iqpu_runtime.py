...
class IQPU:
    ...
    def run_qcl_v6_fused(
        ...
        """Fused torch 版 QCL v6：整个时间演化在 GPU 一次完成。

        用 batched matmul 替代 collapse operator Python 循环，
        参考 FlashMLA 减少 Python↔GPU 搬运次数。

        参数与 run_qcl_v6 完全相同。
        """
        import torch
        from .lindblad_torch import FusedLindbladSolver, _np_to_torch
        ...
        eff_t_max = t_max_override if t_max_override is not None else cfg.t_max
        t3 = min(eff_t_max, t2 + boost_duration)
        steps = int(np.ceil(eff_t_max / dt))

        step_t1 = int(t1 / dt)
        step_t2 = int(t2 / dt)
        step_t3 = int(t3 / dt)

        # 构建 phase list: (end_step, H, collapse_cache)
        H_phases = [
            (step_t1, H_base_np, c_pcm_cpu),
            (step_t2, H_pulse_np, c_qim_cpu),
            (step_t3, H_boost_np, c_boost_cpu),
            (steps, H_base_np, c_pcm_cpu),
        ]

        # 运行 fused solver
        solver = FusedLindbladSolver(self.DIM, device=torch_dev, dtype=torch_dtype)

        t_start = time.time()
        rho_final_torch, obs_log = solver.run(
            rho_init=rho_np,
            dt=dt,
            steps=steps,
            H_phases=H_phases,
            obs_every=self._obs_every,
        )
        elapsed = time.time() - t_start
        ...
        return IQPURunResult(
            ...
            rel_phase=np.array([], dtype=np.float64),
            C_log=np.array([], dtype=np.float64),
            neg_log=None,
            final_sz=final_sz,
            final_n=final_n,
            final_rel_phase=final_rel_phase,
